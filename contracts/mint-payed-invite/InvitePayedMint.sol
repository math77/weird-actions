...
import {InvitePayedMintStorage} from "./InvitePayedMintStorage.sol";
...
/// @title InviteMint 
/// @author matheus
contract InvitePayedMint is IInvitePayedMint, InvitePayedMintStorage, ERC721AUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
  ...
  /// @notice Checks if the desired quantity to mint + giving not exceed the max supply
  ...
  function mint(
    ...
    payable 
    nonReentrant
    hasSupplyLeft(quantity + giving)
    returns (uint256) 
  {

    // I AM NOT SATISFIED WITH THIS FLOW MINT. RETHINK FROM SCRATCH
    ...
    /*
      Use a param or put everything in msg.value?
      msg.value = minting user tokens + paying to next minter
    */

    if(paying != (mintPrice * giving)) revert WrongValue({correctValue: mintPrice * giving});
    ...
    invitations[inviting] = IInvitePayedMint.Invitation({
  ...
  function statsPerAddress(address minter) external view override returns (IInvitePayedMint.AddressMinterDetails memory) {
    return 
      IInvitePayedMint.AddressMinterDetails({
  ...
  function mintDetails() external view returns (IInvitePayedMint.MintDetails memory) {
    return 
      IInvitePayedMint.MintDetails({
  ...
  function supportsInterface(bytes4 interfaceId) public view override(ERC721AUpgradeable, AccessControlUpgradeable) returns (bool) {
    return 
      ...
      type(IInvitePayedMint).interfaceId == interfaceId;
  }
}